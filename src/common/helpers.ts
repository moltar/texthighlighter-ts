...
import { optionsI, hlDescriptorI, paramsImp, paramsClass } from "../types";
import {
  TIMESTAMP_ATTR,
  IGNORE_TAGS,
  NODE_TYPE,
 ...
 * @static
 */
function createWrapper(options: optionsI) {
 ...
 * @memberof TextHighlighter
 */
export const isHighlight = function (el: HTMLElement) {
  return (
 ...
 * @param {HTMLElement[]} highlights - highlights to flatten.
 * @memberof TextHighlighter
 */
export const flattenNestedHighlights = function (highlights: HTMLElement[]) {
  let again;
  // self = this;
 ...
 * @memberof TextHighlighter
 */
export const mergeSiblingHighlights = function (highlights: HTMLElement[]) {
  //   const self = this;
 ...
 * @memberof TextHighlighter
 */
export const normalizeHighlights = function (highlights: HTMLElement[]) {
  ...
  if (wnd) {
    ...
    if ("find" in wnd) {
      ...
      while (textRange.findText(text, 1, caseSens ? 4 : 0)) {
        if (
          !dom(el).contains(textRange.parentElement()) &&
          textRange.parentElement() !== el
        ) {
          break;
        }
...
export const getHighlights = function (el: HTMLElement, params?: paramsImp) {
  if (!params) params = new paramsClass();
...
const removeHighlights = function (element: HTMLElement, options?: optionsI) {
