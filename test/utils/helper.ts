...
export async function getExpectedDict<T>(
  testData: string
): Promise<Record<string, T | null>> {
  const filePath = path.join(base_DIR, testData + ".json");
  console.log(filePath);
  if (fs.existsSync(filePath)) {
    const expectedResults: Record<string, T> = await JSON.parse(
...
export async function getExpected<T>(
  testData: string,
  key: string
): Promise<T | null> {
...
export async function updateExpected<T>(
  testData: string,
  key: string,
  result: T | null
) {
  ...
  fs.writeFileSync(filePath, stringResults);
}
